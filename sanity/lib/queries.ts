...
export const HOME_RESOURCES_QUERY = defineQuery(
	`*[_type == "resource" && programContent != true]| order(_updatedAt desc)[0...8]{slug, title, "imageURL": headerImage.asset->url}`
);

export const HOME_BLOGS_QUERY = defineQuery(
	`*[_type == "blog"]| order(_updatedAt desc)[0...8]{link, title, "imageURL": cardImage.asset->url}`
);
