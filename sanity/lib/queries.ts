...
export const HOME_RESOURCES_QUERY = defineQuery(
  `*[_type == "resource" && abahContent != true]| order(_updatedAt desc)[0...4]{slug, title, "imageURL": headerImage.asset->url}`
);

//query that returns the 4 most recently modified activities
export const HOME_ACTIVITIES_QUERY = defineQuery(
  `*[_type == "activity" && abahContent != true]| order(_updatedAt desc)[0...4]{slug, title, "imageURL": headerImage.asset->url}`
);
...
export const RESOURCES_QUERY = defineQuery(
  `*[_type == "resource" && abahContent != true]| order(_createdAt desc){slug, title, "imageURL": headerImage.asset->url}`
);
...
export const ACTIVITIES_QUERY = defineQuery(
  `*[_type == "activity" && abahContent != true]| order(_updatedAt desc){slug, title, "imageURL": headerImage.asset->url}`
);
